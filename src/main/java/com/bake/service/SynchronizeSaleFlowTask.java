...
public class SynchronizeSaleFlowTask extends BaseTask{
    ...
    protected JsonArray getSubData(String time) {
        String sql = "SELECT  p.branch_no ,p.com_no,DATE_FORMAT(p.oper_date, '%Y-%m-%d %H:%i:%s') as oper_date,p.sale_money,p.sale_qnty,p.flow_no,p.item_no,p.sale_man,d.giv_amt,p.oper_id,p.sell_way FROM  "+ Sysconfig.dbName+".t_rm_saleflow p left join  (select s.giv_amt,p.com_no from "+Sysconfig.dbName+".t_rm_daysum s,"+Sysconfig.dbName+".t_rm_saleflow p where s.item_no=p.item_no and p.sell_way='C') d on  d.com_no=p.com_no where p.oper_date>'"+time+"'";
    ...
    protected void putSubPath(JSONArray dates, JsonObject data) {
        ...
        str[8] = data.get("sale_man")==null?"0":data.get("sale_man").getAsString();
